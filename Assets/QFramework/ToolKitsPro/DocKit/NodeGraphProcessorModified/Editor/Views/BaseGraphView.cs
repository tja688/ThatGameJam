...
namespace GraphProcessor
{
    ...
    public class BaseGraphView : GraphView, IDisposable
    {
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            {
                evt.menu.AppendAction(LocaleKitEditor.IsCN.Value ? "创建节点" : "Create Node",
                    d =>
                    {
                        
                        this.ReflectionCallPrivateMethod<GraphView>("OnContextMenuNodeCreate", d);
                    },
                    DropdownMenuAction.AlwaysEnabled);
                
                Vector2 position =
                    (evt.currentTarget as VisualElement).ChangeCoordinatesTo(contentViewContainer, evt.localMousePosition);
                evt.menu.AppendAction(LocaleKitEditor.IsCN.Value ? "创建分组" : "Create Group",
                    e => AddSelectionsToGroup(AddGroup(new Group("Create Group", position))),
                    DropdownMenuAction.AlwaysEnabled);
                
                
                evt.menu.AppendAction(LocaleKitEditor.IsCN.Value ? "重新加载" : "Reload",
                    d =>
                    {
 ...
 (evt.currentTarget as VisualElement).ChangeCoordinatesTo(contentViewContainer, evt.localMousePosition);
        ...
        {
            if (e.keyCode == KeyCode.S && e.commandKey || e.keyCode == KeyCode.S && e.ctrlKey)
            {
                SaveGraphToDisk();
                e.StopPropagation();
            }
            ...
            else if (e.keyCode == KeyCode.V && e.commandKey || e.keyCode == KeyCode.V && e.ctrlKey)
            {
                Debug.Log(EditorGUIUtility.systemCopyBuffer);
                Debug.Log(EditorGUIUtility.systemCopyBuffer.Length);
                Debug.Log(GUIUtility.systemCopyBuffer);
                Debug.Log(GUIUtility.systemCopyBuffer.Length);

                // if (Clipboard.ContainsImage())
                // {
                // 	Debug.Log("ContainsImage");
                // }
            }
        }
        ...
        private DateTime mPreviousCountTime;
        ...
        void MouseDownCallback(MouseDownEvent e)
        {
            ...
            if (e.button == 0)
            {
                if (DateTime.Now - mPreviousCountTime < TimeSpan.FromSeconds(0.5f))
                {
                    OnDoubleClicked?.Invoke();

                    this.RemoveManipulator(mRectangleSelector);

                    schedule.Execute(() =>
                    {
                        mRectangleSelector = new RectangleSelector();
                        this.AddManipulator(mRectangleSelector);
                    }).ExecuteLater(300);

                    return;
                }

                mPreviousCountTime = DateTime.Now;
            }
            else
            {
                // break double click
                mPreviousCountTime = DateTime.Now - TimeSpan.FromSeconds(1000);
            }
        }
        ...
        private RectangleSelector mRectangleSelector;
        ...
        /// </summary>
        public void DelayedDeleteSelection() => schedule
