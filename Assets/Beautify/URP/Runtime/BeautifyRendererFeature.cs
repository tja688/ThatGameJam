            ...
            static void FullScreenBlitToCamera (CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier destination, Material material, int passIndex) {
                ...
                if (destination == BuiltinRenderTextureType.CameraTarget) {
...
#endif
                    {
                        ...
                        int tempRT = Shader.PropertyToID("_BeautifyTempBlit");
                        bool useBilinear = beautify != null && beautify.downsampling.value && beautify.downsamplingMultiplier.value > 1f && beautify.downsamplingBilinear.value;
                        cmd.GetTemporaryRT(tempRT, sourceDesc, useBilinear ? FilterMode.Bilinear : FilterMode.Point);
                        FullScreenBlit(cmd, source, tempRT, material, passIndex);
                        FullScreenBlit(cmd, tempRT, destination, material, useBilinear ? (int)Pass.CopyBilinear : (int)Pass.CopyExact);
...
#pragma warning restore 0618
        ...
        class BeautifyOutlineDepthPrepass : ScriptableRenderPass {
            ...
            public override void Configure (CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor) {
                ...
                RTHandle outlineDepthHandle = RTHandles.Alloc(rti, name: outlineDepthRT);
