# RECIPES — Executable Workflows (v4)
...
- `VERIFY` (steps)
- `FEATURE_NOTE` (path + what changed)
...
6. **Write FeatureNote.md (MUST)**

- Create/update: `Assets/Scripts/Features/[FeatureName]/FeatureNote.md`
- Use the template in `TEMPLATES.md` (“FeatureNote.md”).
- Focus on: purpose, runtime wiring (Unity + `GameRootApp`), public API surface (Events / Commands / Queries / Bindables), and 1–2 minimal usage examples.

7. **Report**

- Output `CHANGES`, `VERIFY`, `FEATURE_NOTE`, `NOTES`.

---

## 1.5 FeatureNote.md (Feature Integration Notes)

### When to use

You completed a Feature, or changed a Feature’s **public integration surface** (Events / Commands / Queries / Bindables), or changed how the Feature is wired in Unity.

### Goals (MUST)

- Keep the note short and skimmable (API-first).
- Answer: **What is it? How do I wire it? What do I listen/send/call?**
...
- **Folder & Key Files**
- **Runtime Wiring**
  - Root registration (`GameRootApp`)
  - Scene setup (Unity) — what Controllers must exist
- **Public API Surface**
  - Events (outbound)
  - Request Events (inbound write requests, if used)
  - Commands (write path)
  - Queries (read path, if used)
  - Model read surface (Bindables / readonly properties)
- **Typical Integrations** (1–2 examples)
- **Verify Checklist**
- **UNVERIFIED** (only if needed)

### Output requirement

- The agent MUST create/update `Assets/Scripts/Features/[FeatureName]/FeatureNote.md` and mention it in `CHANGES`.
