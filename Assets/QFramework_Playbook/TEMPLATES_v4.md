...
## 4. FeatureNote.md (Feature Integration Notes)

**File (required per Feature):**

- `Assets/Scripts/Features/[FeatureName]/FeatureNote.md`

**Intent:**
A short, skimmable note that documents the Feature’s **public integration surface** for humans and future agents.

```md
# Feature: [FeatureName]

## 1. Purpose
- (1–3 lines) What this Feature does and why it exists.

## 2. Folder & Key Files
- Root: `Assets/Scripts/Features/[FeatureName]/`
- Controllers:
  - `[...Controller.cs]` — Unity scene entry points / bindings
- Systems:
  - `I[Feature]System`, `[Feature]System` — orchestration
- Models:
  - `I[Feature]Model`, `[Feature]Model` — state + bindables
- Utilities (if any):
  - `I[Feature]Utility`, `[Feature]Utility` — persistence / external services
...
- Registered modules:
  - Model: `I...Model`
  - System: `I...System`
  - Utility: `I...Utility` (if any)

### 3.2 Scene setup (Unity)
- Required MonoBehaviours:
  - `...Controller` on `...` (where/why)
- Inspector fields (if any):
  - `...` — meaning / recommended value
...
- `struct ...Event`
  ...
  - Example:
    ```csharp
    this.RegisterEvent<...Event>(On...)
        .UnRegisterWhenDisabled(gameObject);
    ```

### 4.2 Request Events (Inbound write requests, optional)
> Use when a System-originated trigger must result in a Command execution
- `struct Request...CommandEvent`
  - Who sends it:
  - Who handles it (Controller → Command):
...
- Example: On respawn → refill health (listen to `PlayerRespawnedEvent` → send `RefillHealthCommand`)
